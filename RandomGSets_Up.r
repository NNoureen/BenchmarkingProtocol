...
UpGenesList <-  read.table('IDHastrocytoma_MAST_DEGS_15March2021.txt',sep='\t',head=T)   ### read the DEGS file generated using MAST function
UpGenesList = UpGenesList[which(UpGenesList$avg_logFC > 0),]                             ### Shortlist the upregulated genes based on logFC values > 0
NoiseGenesList <- genes[-which(genes %in% UpGenesList)] 								 ### Take remaining genes as noise list	
...
OriginalSet = sample(UpGenesList,L1)
OS = c(originalid,"Random",OriginalSet)


sink("IDHAstrocytoma_RS_Up.gmt",append=T)       ### save the Up regulated gold standard data set
writeLines(OS,sep='\t')
writeLines("",sep='\n')
sink()
...
sink("IDHAstrocytoma_RS_Up.gmt",append = T)
